...
speedBot.post("/", async (req, res) => {
  try {
    ...
    if (!domain || !domain.startsWith("http")) {
      return res.status(400).json({ error: "Please include a valid URL starting with http/https" });
    }

    const recommendations = await analyzeWebsiteAndGetRecommendations(domain);
